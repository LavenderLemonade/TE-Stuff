...
namespace TenmoServer.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        [HttpGet("{id}")]
        public ActionResult<List<string>> ListUsers(int id)
        {
            List<string> newList = new List<string>();

            List<User> allOfEm = userDao.ListThemUsers(id);

            foreach(User ppl in allOfEm)
            {
                newList.Add($"User ID: {ppl.UserId} --- Username: {ppl.Username}");
            }
